  ...
  return (
    <div className="bg-white  rounded-lg shadow-md p-10 border border-2 border-[#ba1c21]">
      <div className="flex flex-col items-center mx-24 rounded-sm bg-zinc-50">
        <h3 className="text-[#003058] text-lg font-bold mb-2">
          {buildings.name}
        </h3>
        <p className="mb-2 font-bold text-md">
          Number of rooms: {buildings.rooms}
        </p>
      </div>
      <br />
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border p-2 rounded-lg mb-4 ${
                !roomNumber ? "border-red-500" : "border-gray-300"
              }`}
      ...
      {showRoomDetails && (
        <div className="bg-[#003058] border border-gray-300 rounded-md p-4 grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 gap-2 ">
          {rooms &&
            rooms.map(({ _id, roomNumber, roomType, roomPaints }) => (
              <RoomDetails
                key={_id}
